...
import { CalendarIcon, MapPinIcon, ArrowLeftIcon, Loader2, Upload, X } from "lucide-react";
...
export default function EditEvent() {
  ...
  const { updateEvent, uploadEventImage, fetchEvent, currentEvent, isLoading } = useEventStore();
  ...
  const [imageSource, setImageSource] = useState("url"); // Default to URL if editing
  ...
  useEffect(() => {
    ...
    if (!user || !user.isAdmin) {
      toast.error("You must be an admin to edit events");
      navigate("/");
    }
  ...
  useEffect(() => {
    if (currentEvent && currentEvent.id === id) {
      ...
      setPrice(currentEvent.price?.toString() || "0");
      setTicketLink(currentEvent.ticketLink || "");
      setSupportsScanning(currentEvent.supportsScanning || false);
      setCategory(currentEvent.category || "general");
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Tabs defaultValue="file" value={imageSource} onValueChange={setImageSource} className="w-full">
